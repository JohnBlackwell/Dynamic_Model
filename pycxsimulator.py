...
class GUI:
    ...
    def resetModel(self):
        ...
        self.runPauseString.set("Run")
        self.modelInitFunc()
        self.modelPopulate() #popluate after init
        self.currentStep = 0;
    ...
    def start(self,func=[]):
        if len(func)==4:
            ...
            self.modelPopulate = func[1]
            self.modelDrawFunc = func[2]
            self.modelImpFunc =  func[3]

            if (self.modelImpFunc.__doc__ != None and len(self.modelImpFunc.__doc__)>0):
                self.showHelp(self.buttonStep,self.modelImpFunc.__doc__.strip())
            ...
            self.modelInitFunc()
            self.modelPopulate()   #populate the grid
            self.drawModel()
